            ...
            steps {
                script {
                    ...
                    withEnv(['JENKINS_NODE_COOKIE=dontKillMe']) {
                        sh '''
                            set -e
                            # 修复：用步骤名称作为停止文件标识，避免PID匹配错误
                            keep_alive() {
                                local step_name=$1
                                local stop_flag="/tmp/keep_alive_${step_name}.stop"
                                # 删除旧的停止文件（避免残留）
                                rm -f "$stop_flag"
                                # 后台心跳进程：检测到停止文件就退出
                                (
                                    ...
                                    rm -f "$stop_flag"  # 清理停止文件
                                ) &
                                echo $!  # 返回后台进程PID，用于后续kill
                            }
                            ...
                            stop_keep_alive() {
                                ...
                                local pid=$2
                                local stop_flag="/tmp/keep_alive_${step_name}.stop"
                                touch "$stop_flag"  # 创建停止文件，触发心跳进程退出
                                wait $pid 2>/dev/null || true  # 等待心跳进程正常退出
                            }

                            echo ">>> 准备部署 ${SERVICE_NAME} ... [步骤1/5]"
                            # 步骤1：创建部署目录（启动心跳，传入步骤名称）
                            HB_PID=$(keep_alive "创建部署目录")
                            echo ">>> 步骤1/5：创建部署目录 ${DEPLOY_DIR} ..."
                            mkdir -p ${DEPLOY_DIR}
                            echo ">>> 步骤1/5：目录创建完成 ✔️"
                            stop_keep_alive "创建部署目录" $HB_PID  # 停止心跳，传入步骤名称+PID

                            # 步骤2：检查并停止旧进程（核心修复：循环替代sleep 1）
